  ...
  return (
    <div className='m-5 p-5 border-box w-full'>
      ...
      <form className="grid grid-cols-3 gap-4 mt-5">
        <Formfield value={collegeState.name} onChange={handleChange} className={'col-span-1'} name={"name"} type={"text"} id={"name"} title={"Title"} placeholder={"Enter the notice"} required={true} />
        <Formfield value={collegeState.address} onChange={handleChange} className={'col-span-1'} name={"name"} type={"text"} id={"id"} title={"Date*"} placeholder={"Enter date in yyyy-mm-dd format"} required={true} />
